...
module OutlookMcp
  module Tools
    class ReplyAllToEmail < MCP::Tool
      ...
      class << self
        def call(id:, comment:, server_context:, **)
          ...
          graph.reply_all_to_message(id, comment: comment)
          MCP::Tool::Response.new([{type: "text", text: "Reply-all sent."}])
        rescue Faraday::Error => e
          MCP::Tool::Response.new([{type: "text", text: "Error replying all: #{e.message}"}], error: true)
        end
      end
    end
  end
